...
function cau3()
{
    ...
    {
        filter.jobTitle = find[i].jobTitle;
        filter.locations = find[i].locations;
        filter.jobSalary = find[i].jobSalary;
        filter.benefits = find[i].benefits;
        filter.skills = find[i].skills;
        filter.jobRequirement = find[i].jobRequirement;
        console.log(filter);
    }
}
...
function cau4()
{
    ...
    while(true)
    {
        let eKeyword = prompt('Enter the keyword');
        if(eKeyword=='')
        {
            alert('Your keyword is null')
            continue;
        }
        else if(eKeyword==null)
        {
            let cf = confirm('Exit');
            if(cf)
            {
        ...
        eKeyword = eKeyword.toLowerCase();
        for(i=0;i<dictionary.length;i++)
        {
            let exp = dictionary[i];
            var x = 0;
            if(exp.keyword == eKeyword)
            {
                alert(`${eKeyword}\r\n${exp.explanation}`);
                x=1;
                break;
        ...
        if(x==0)
        {
            let newExp = prompt(`We could not find your word: ${eKeyword}, leave your explanation`);
            if(newExp)
            {
                var newitem = {
                    keyword:eKeyword,
